import { resolve, join, parse, dirname, isAbsolute } from "node:path";
import figlet from "figlet";
import { normalizePath, version } from "vite";
import picocolors from "picocolors";
import crypto from "node:crypto";
import { inspect } from "node:util";
import { LogFactory, Level, PatternLayout } from "base-log-factory";
...
function getDepsCacheDirPrefix(config) {
  return normalizePath(resolve(config.cacheDir, "deps"));
}
...
const logFactory = new LogFactory({
  ...
  appenders: [
    new DebugAppender({
      layout: new PatternLayout("%d{HH:mm:ss} [%p] - %m", createConverter)
    })
  ]
});
