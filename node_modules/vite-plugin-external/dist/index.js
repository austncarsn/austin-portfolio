...
function makeStashFilePath(cacheDir, libName) {
  return node_path.join(cacheDir, `${vpRuntimeHelper.flattenId(libName)}.js`);
}
...
async function stash(libName, globalName, cacheDir) {
  ...
  let info;
  let code;
  if (isWhatType.isAbsoluteURL(globalName)) {
    info = {
      external: libName,
      resolvedId: libPath,
      link: globalName,
  ...
  await fsExtra.outputFile(libPath, code, "utf-8");
  return info;
}
class Resolver {
...
async function setAliases(opts, config) {
  ...
  if (!isWhatType.isPlainObject(externals)) {
  ...
  const { cacheDir } = opts;
  let alias = vpRuntimeHelper.getValue(config, "resolve.alias", []);
...
class ExternalHook {
  ...
  use(arg) {
    ...
    if (isWhatType.ensureType(arg, type === "boolean")) {
      hook = () => arg;
    } else if (isWhatType.ensureType(arg, type === "function")) {
      hook = (id, importer, isResolved) => !id.startsWith("\0") && arg(id, importer, isResolved);
    } else if (isWhatType.isPlainObject(arg)) {
      hook = (id) => arg[id];
    } else if (arg) {
...
function rollupOutputGlobals(output, globalObject) {
  ...
  output.globals = (libName) => {
    ...
    if (!globalName) {
      if (isWhatType.isFunction(originalGlobals)) {
        globalName = originalGlobals(libName);
      } else if (isWhatType.isObject(originalGlobals)) {
        globalName = originalGlobals[libName];
      }
    }
...
function setOutputGlobals(rollupOptions, globalObject, opts) {
  ...
  if (isWhatType.isFunction(externalGlobals)) {
    ...
    const output = vpRuntimeHelper.getValue(rollupOptions, "output", {});
...
function setExternals(opts, config) {
  ...
  const rollupOptions = vpRuntimeHelper.getValue(config, "build.rollupOptions", {});
  const { nodeBuiltins, externalizeDeps, command } = opts;
  if (command === "build") {
    if (nodeBuiltins) {
      let builtinModuleArray;
      externalHook.use(builtinModuleArray = node_module.builtinModules.map((builtinModule) => {
        return new RegExp(`^(?:node:)?${vpRuntimeHelper.escapeRegex(builtinModule)}(?:/.+)*$`);
      }));
    ...
    if (externalizeDeps) {
      ...
      externalHook.use(deps = externalizeDeps.map((dep) => {
        return node_util.types.isRegExp(dep) ? dep : new RegExp(`^${vpRuntimeHelper.escapeRegex(dep)}(?:/.+)*$`);
      }));
      ...
      if (value) {
        switch (key) {
          ...
          case "logLevel":
            ...
            break;
          case "externals":
            if (isWhatType.isPlainObject(value)) {
...
async function cleanupCache(deps, config) {
  ...
  const depsCacheDir = vpRuntimeHelper.getDepsCacheDir(config, ssr);
  ...
  return {
    ...
    setup(build) {
      ...
      build.onLoad({ filter: /./, namespace: externalWithConversionNamespace }, (args) => {
        ...
        return {
          contents: vite.isCSSRequest(args.path) ? `import ${modulePath};` : `export { default } from ${modulePath};
...
async function setOptimizeDeps(resolver, opts, config) {
  const plugins = vpRuntimeHelper.getValue(config, "optimizeDeps.esbuildOptions.plugins", []);
...
function pluginExternal(opts) {
  ...
  const version2 = vpRuntimeHelper.getRuntimeVersion();
  vpRuntimeHelper.colorful.green(`${node_os.EOL}Vite@${version2} ${pkg.name}@${pkg.version}`);
  const plugin = {
    name: PLUGIN_NAME,
